...
export default function CharacterListScreen(navigation: RootTabScreenProps<'Characters'>) {
    const ironManWithCustomThumbnail: Character = {
        ...
        thumbnail: {
            path: "https://static.wikia.nocookie.net/marvelcinematicuniverse/images/3/35/IronMan-EndgameProfile.jpg/revision/latest/scale-to-width-down/699?cb=20190423175213",
            extension: "jpg"
        },
        ...
        urls: [
            {
                type: "detail",
                url: "http://marvel.com/characters/29/iron_man?utm_campaign=apiRef&utm_source=6bd6e32104f82a08a7c15b87910f5302"
            },
            {
                type: "wiki",
                url: "http://marvel.com/universe/Iron_Man_(Anthony_Stark)?utm_campaign=apiRef&utm_source=6bd6e32104f82a08a7c15b87910f5302"
            },
            {
                type: "comiclink",
                url: "http://marvel.com/comics/characters/1009368/iron_man?utm_campaign=apiRef&utm_source=6bd6e32104f82a08a7c15b87910f5302"
            }
        ]
    };
    const [characters, setCharacters] = React.useState<Character[]>([ironManWithCustomThumbnail]);

    let fetchCharacters = debounce(async () => {
        const timestamp = Date.now();
        const hash = MD5(timestamp + privateKey + publicKey);
        const url = 'http://gateway.marvel.com/v1/public/characters';
        const urlWithParams = `${url}?ts=${timestamp}&apikey=${publicKey}&hash=${hash}`
        ...
        await fetch(urlWithParams, {
            ...
            if (r.status == 200) {
                r.json().then(body => setCharacters(body.data.results));
            } else {
